...
public class KafkaSender {
	...
	//新kafka
	public static final String KAFKA_BOOTSTRAP_SERVERS = "192.168.111.30:6667,192.168.111.31:6667,192.168.111.32:6667";
//	public static final String KAFKA_BOOTSTRAP_SERVERS = "104.250.147.226:6667,104.250.147.106:6667,104.250.150.18:6667";

	private static String kafkaStoredMessageBaseDir = "/home/kok/app/ssp/ssp_backup/ROOT/kafka_stored_message";
...
//	public static final String KAFKA_BOOTSTRAP_SERVERS = "node14:9222";
	...
	private static final int LOG_INTERVAL_TIME_MS = 1000*10;// 打印日志时间间隔
...
//		Thread.sleep(100000000000L);
	...
	volatile private KafkaProducer<String, String> producer;

	private volatile KafkaStoredMessageManager kafkaStoredMessageManager = null;

	volatile private Map<Long, Long> sendRequests = new HashMap<Long, Long>();
	volatile private Map<Long, Long> sendFailures = new HashMap<Long, Long>();

	volatile private Map<Long, Long> reSendRequests = new HashMap<Long, Long>();
	volatile private Map<Long, Long> sendSuccesses = new HashMap<Long, Long>();
	private Map<String, Object> customProducerConfig = new HashMap<String, Object> ();

	volatile private static Map<String, KafkaSender> SINGLETON_INSTANCE_MAP = new HashMap<String, KafkaSender>();

	public static synchronized KafkaSender instance(String topic){
		KafkaSender ks = SINGLETON_INSTANCE_MAP.get(topic);
		if(ks == null) {
			ks = new KafkaSender(topic);
...
//		initHeartbeat();
	...
	public KafkaSender(final String topic, String sendFailMessagesStoreSubDir, Level logLevel, Map<String, Object> customProducerConfig){

		LOG.setLevel(logLevel);
		Logger.getLogger("org.apache.kafka").setLevel(logLevel);
		this.topic = topic;
		kafkaStoredMessageManager = new KafkaStoredMessageManager(topic, kafkaStoredMessageBaseDir + java.io.File.separator + sendFailMessagesStoreSubDir, logLevel);
...
//				LOG.info("Sending to kafka " + size + " message(s)");
			...
			if(System.currentTimeMillis() - lastLogTime5 > LOG_INTERVAL_TIME_MS) {
				logInfo("Send batch use time: ", (System.currentTimeMillis() - currBatchSendTime)/1000F, "s, count: ", messages.length);
//				LOG.info("Send batch use time: " + (System.currentTimeMillis() - currBatchSendTime)/1000F+"s, count: " + messages.length );
	...
	private void reSend(String key, String... messages) {
		try {
...
//				LOG.info("Send batch use time: " + (System.currentTimeMillis() - currBatchReSendStartTime)/1000F+"s, count: " + messages.length );
				logInfo("Send batch use time: ", (System.currentTimeMillis() - currBatchReSendStartTime)/1000F, "s, count: ", messages.length);
				lastLogTime5 = System.currentTimeMillis();
			}
...
//		props.put(ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG, 100*5);
...
//		props.put(ProducerConfig.MAX_BLOCK_MS_CONFIG, 10 /*Long.MAX_VALUE*/);
...
//		}
	...
	public static class KafkaStoredMessageManager {
		...
		private static final Logger LOG = Logger.getLogger(KafkaStoredMessageManager.class);
		private static final String RECORD_SEPARATOR = "\r\n\r\n";
		private static final Object WRITE_LOCK = new Object();
		private static final Object READ_LOCK = new Object();

		private static final String FILE_NAME_TOPIC_SEPARATOR = "-";
...
//							LOG.warn("Delete expired files fail", t);
		...
		public void retrieve(final KafkaSender.KafkaStoredMessageManagerRetrieveCallback kafkaStoredMessageAccessorRetrieveCallback) throws IOException {
...
//						LOG.info("Retrieve messages from reading file: " + f.getAbsolutePath());
						...
						List<KafkaSender.KafkaStoredMessage> list = new ArrayList<KafkaSender.KafkaStoredMessage>();
...
//								LOG.debug("Read plain reading file content:\n" + buff.toString() + "\npath:\n" + f.getAbsolutePath());
							...
							for(String o : buff.toString().split(RECORD_SEPARATOR)) {
								message = o;
								list.add(JSON.toBean(message, KafkaSender.KafkaStoredMessage.class));
							}
...
//				}
		...
		public void store(final String topic, final KafkaSender.KafkaStoredMessage storedKafkaMessage) {
			synchronizedCall(new Callback() {
				public void onCallback() {
					...
					try {
						...
						if(LOG.isDebugEnabled()) {
							logDebug("Failed msg stored:\n", msg, "path:\n", currOutputStreamPath);
//							LOG.debug("Failed msg stored:\n" + msg + "path:\n" + currOutputStreamPath);
						}
...
//		LOG.info("Synchronized call "+from+" use time: " + (new Date().getTime() - currBatchSendTime)/1000F+"s");
		...
		public synchronized OutputStream generateOutputStream(String topic) throws IOException {
			...
			if(lastWritingSignCreateTime == null
				...
				String writingSign = DF.format(new Date());
...
//		}
			...
			public  synchronized  void listColsedProcess(ProcessMonitor.CallBack callback){
...
//                fc = new FileOutputStream(f, true).getChannel();
						lock = fc.tryLock();
						if(lock != null) {
							logWarn("Get list lock success via thread[", pid(), ":", Thread.currentThread().getId(), "], file: ", f.getAbsolutePath(), ", lock: ", lock);
...
//                		System.out.println(buff);
						...
						logWarn("Get list lock fail via thread[", pid(), ":", Thread.currentThread().getId(), "], file: ", f.getAbsolutePath(), ", because: OverlappingFileLockException");
