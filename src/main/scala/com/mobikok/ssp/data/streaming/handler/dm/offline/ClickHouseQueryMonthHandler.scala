...
class ClickHouseQueryMonthHandler extends Handler {
  ...
  override def handle(): Unit = {
    ...
    RunAgainIfError.run {
      viewConsumerTopics.foreach { topic =>
        ...
        clickHouseClient.hiveWriteToClickHouse("ssp_report_overall_dm_month", topic._1, ms.map{ b_time => b_time.value })
