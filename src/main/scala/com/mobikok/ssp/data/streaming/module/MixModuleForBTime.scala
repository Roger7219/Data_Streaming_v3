...
/**
  ...
  */
class MixModuleForBTime (config: Config,
...
//    mixModulesBatchController.isMaster(moduleName)
//  }
//  override def getName(): String = moduleName
//  override def isInitable(): Boolean = {
//    mixModulesBatchController.isInitable(moduleName)
////    var result = false
...
//    mixModulesBatchController.isRunnable(moduleName)
...
//        if (isEnableDwi) LOG.warn("Filtered Rdd take(2)", filtered.take(2))
        ...
        if (isFastPollingEnable && filtered.isEmpty() && GlobalAppRunningStatusV2.isPreviousRunning(concurrentGroup, moduleName)) {
          ...
          if (isEnableDwiUuid) {
            ...
            // cacheUuidRepeats = uuidFilter.filter(dwi).alias("ur")
            dwi = uuidFilter.filter(dwi).alias("dwi")
            //            uuidFilter.dwiHandle(dwi)
            //            uuidFilter.dwrHandle(dwr)
            //          }
          ...
          //            dwiLTimeSourceField = "dwiLoadTime"
...
//          moduleTracer.trace("refresh cacheDwi")
          ...
          if (isEnableDwr) {
            if (!dwrIncludeRepeated) {
              newDwi = newDwi.where(uuidFilter.dwrNonRepeatedWhere() /*"repeated = 'N'"*/)
            }
...
//              LOG.warn("master threadSafeDwr take(2)", shareDwrTableModulesBatchController.get().take(2))
...
//          var dwrCount = 0L
//          if (isEnableDwr) {
//            dwrCount = shareDwrTableModulesBatchController.get().count()
//          }
...
//            val lastTime  = if(!x.isEmpty && x.size > 1) x.tail.map(_.getKeyBody).head else x.map(_.getKeyBody).head
          ...
          if (isEnablePhoenixDm && isMaster && /*dwrCount >0 */ dwiCount > 0) {
...
//            messageClient.pushMessage(new MessagePushReq(HBASE_WRITECOUNT_BATCH_TOPIC, dwiCount ))
          ...
          if (isEnableDwr && dwrGreenplumEnable && /*dwrCount > 0*/ dwiCount > 0) {
          ...
          //Greenplum
          //------------------------------------------------------------------------------------
          if (isEnableDwr && dmGreenplumEenable && /*dwrCount > 0*/ dwiCount > 0) {
          ...
          if (isEnableKafkaDm && isMaster && /*dwrCount > 0*/ dwiCount > 0) {
...
//                  Thread.sleep(24 * 60 * 60 * 1000L)
          ...
          if (isEnableDwr && isMaster) pDwr.unpersist()
        ...
        case e:Exception=> throw new ModuleException(s"${classOf[MixModuleForBTime].getSimpleName} '$moduleName' execution failed !! ", e)
      }
    }
  }
