  ...
  def initDwiHandlers(): Unit = {
    ...
    if (config.hasPath(s"modules.$moduleName.dwi.handler")) {
      dwiHandlers = config.getConfigList(s"modules.$moduleName.dwi.handler").map { x =>
        val hc = x
        val h = Class.forName(hc.getString("class")).newInstance().asInstanceOf[com.mobikok.ssp.data.streaming.handler.dwi.Handler]
//        var col = "null"
...
//                    if (handler.isInstanceOf[Persistence]) {
//                      val h = handler.asInstanceOf[Persistence]
//                      h.transactionParentId = parentTid
//                    }
////                    handler match {
////                      case persistence: Persistence =>
////                        persistence.transactionParentId = parentTid
////                      case _ =>
////                    }
...
//                    if (!isMaster && dwrHandlers.nonEmpty) {
//                      LOG.warn(dwrHandlers.head.getClass.getName)
//                      throw new ModuleException("Module of include 'dwr.handler' must config: master=true")
//                    }
//                    dwrHandlers.foreach { h =>
...
//                      throw new ModuleException("Module of include 'dwr.handler' must config: master=true")
//                    }
//                    dwrHandlers.foreach { handler =>
//                      if (handler.isInstanceOf[Persistence]) {
//                        handler.asInstanceOf[Persistence].transactionParentId = parentTid
//                      }
//                      if (!handler.isAsynchronous) {
//                        mixModulesBatchController.set({
//                          handler.handle(mixModulesBatchController.get())._2
//                        })
//                      }
//                      LOG.warn(s"Dwr ${handler.getClass.getSimpleName} handle completed")
//                      moduleTracer.trace(s"dwr ${handler.getClass.getSimpleName} handle")
//                    }
//                    // 异步的handler等待dwr全部执行完后再执行
//                    val asynDwrHandlers = dwrHandlers.filter{ handler => handler.isAsynchronous}
//                    if (asynDwrHandlers.nonEmpty) {
//                      dwrHandlerCookies = Array[(String, TransactionCookie)]()
//                      asynDwrHandlers.foreach{ handler =>
//                        ThreadPool.execute {
//                          moduleTracer.startBatch
//                          val (cookieMark, _, cookie) = handler.handle(mixModulesBatchController.get())
//                          dwrHandlerCookies :+ (cookieMark, cookie)
//                          countDownLatch.countDown()
//                        }
//                      }
//                    }
...
//                    if (x._1.isInstanceOf[HiveDWIPersistHandler]) {
//                      x._2.foreach{ y => cacheTransactionCookies(COOKIE_KIND_DWI_T, y)}
//                    } else {
//                      x._2.foreach { y =>
...
//                  dwrHandlerCookies.foreach{ dwrCookie =>
//                    cacheTransactionCookies(dwrCookie._1, dwrCookie._2)
//                  }
//                }
//
//
...
//                  dwrHandlers.filter{ handler => handler.isInstanceOf[Persistence]}.par.foreach{ handler =>
//                    moduleTracer.startBatch
//                    val h = handler.asInstanceOf[Persistence]
//                    h.commit(null) // 不需要传cookie
//                  }
//
//                  // 线程池开始
...
////                  dwiHandlers.filter{ h => h._3.isInstanceOf[Persistence] }.foreach{ h => h._3.commit(null) }
...
//                // Push partitions message
//                //------------------------------------------------------------------------------------
//                var dwrT = null.asInstanceOf[TransactionCookie]
//                if (dwrHandlerCookies != null) {
//                  dwrHandlerCookies.foreach{ h =>
//                    if (h._1.eq("dwrT")) {
//                      dwrT = h._2
//                    }
//                  }
//                }
//                var dwiT = null.asInstanceOf[TransactionCookie]
//                dwiHandlerCookies.foreach{ h =>
//                  if (h._1.getClass.getSimpleName.eq(classOf[HiveDWIPersistHandler].getSimpleName)) {
//                    dwiT = h._2(0)
//                  }
//                }
//                pushChangedHivePartition(dwrT.asInstanceOf[HiveTransactionCookie], dwiT.asInstanceOf[HiveTransactionCookie])
//
...
//
//                if (isEnableDwr && isMaster && dwiCount > 0) {
//                  dwrHandlers.filter{ h => h.isInstanceOf[Persistence] }.foreach{ handler =>
//                    val h = handler.asInstanceOf[Persistence]
//                    h.clean(popNeedCleanTransactions(h.cookieKindMark, parentTid): _*)
//                  }
