...
//            appStates.add(YarnApplicationState.SUBMITTED);
    ...
    public static void killPrevApps(String appName) throws IOException, YarnException {

        yarnClientInit();

        Integer currStartingAppId = null;
        ...
        while (true) {
            ...
            List<ApplicationReport> sameApps = new ArrayList<ApplicationReport>();
            for (ApplicationReport appReport : appsReport) {

                if(appReport.getName().equals(appName)) {
                    sameApps.add(appReport);
                }
            }

            if(sameApps.size() == 1 && currStartingAppId != null && currStartingAppId.equals(sameApps.get(0).getApplicationId().getId())) {
                return;
            }

            sameApps.sort(new Comparator<ApplicationReport>() {
            ...
            //排除最近一次启动的
            if(sameApps.size() > 1) {
                ApplicationReport currStartingApp = sameApps.remove(0);
                currStartingAppId = currStartingApp.getApplicationId().getId();
            }

            //关闭此前的
            for(ApplicationReport k : sameApps) {
                LOG.warn("Killing same prev app: " + k.getApplicationId());
                client.killApplication(k.getApplicationId());
            }
