...
create table adx_ssp_dwi like adx_dsp_dwi;

create table adx_traffic_dwr(
    publisherId    INT,
    appId          INT,
    dspId          INT,
    countryId      INT,
...
-------thirdincome data
drop table if exists adx_thirdincome_dwr;
create table if not exists adx_thirdincome_dwr(
    ...
    clicks              int,
    countryCode         string,
    mediaprice          DECIMAL(19,10)
)
PARTITIONED BY (dataSource STRING, b_date STRING)
STORED AS ORC;
...
-- add on 18/01/09
left join proxy pro         on  pro.id  = am.proxyId
...
select
    ...
    -1                as carrierid,
    dwr.adId          as adid,     --广告位id
    -1                as adtypeid, --广告类型, eg: banner
    'null'            as iab1id,
    'null'            as iab2id,
    0                 as width,
    0                 as height,
    dwr.b_date        as utcday,
    -1                as utchour,

    dwr.sspRequests   as ssprequests,    -- 请求数
    dwr.sspSends      as sspsends,       -- 响应数(下发数)
    0                 as dsprequests,    -- DSP 请求数
    0                 as dspsends,       -- DSP 响应数
    0                 as winnotices,
    dwr.impressions   as impressions,
    dwr.clicks        as clicks,
    ...
    0                 as profit,  -- clearprice - mediaPrice
    ...
    co.id             as companyid,
    co.name           as companyname,
    dwr.datasource    as datasource
from adx_thirdincome_dwr dwr
left join publisher p on  p.id = dwr.publisherId
left join employee am on am.id = p.amId
left join app a on a.id = dwr.appId
left join country c on c.alpha2_code = dwr.countryCode
-- +
left join app_ad aa on aa.id = dwr.adId
