 create temporary view nadx_p_matched_dwi_tmp as
        ...
        from (
          select * from nadx_performance_dwi where repeated = 'N' AND b_time >= date_format(current_timestamp() + INTERVAL -136 HOUR, 'yyyy-MM-dd HH:00:00') AND ( (b_time="2019-06-03 22:00:00") )
        ) pDwi
        left join (
          select * from nadx_traffic_dwi where dataType = 4 AND b_time >= date_format(current_timestamp() + INTERVAL -136 HOUR, 'yyyy-MM-dd HH:00:00') AND ( (b_time="2019-06-03 22:00:00") )
        ) tDwi ON tDwi.b_time = pDwi.b_time AND pDwi.bidid = tDwi.bidRequestId
        left join (
          select * from nadx_performance_dwi where repeated = 'N' AND b_time >= date_format(current_timestamp() + INTERVAL -136 HOUR, 'yyyy-MM-dd HH:00:00') AND ( (b_time="2019-06-03 22:00:00") ) AND type = 'win'
...
select sum(impression_count) from tttt



####
select l_time from nadx_overall_dwr_v2 where b_time ='2019-06-05 16:00:00' group by l_time



select sum(impression_count) from nadx_overall_dwr_v2 where b_time ='2019-06-05 16:00:00' and supply_id =1
select sum(impression_count) from nadx_overall_dwr_v2 where b_time ='2019-06-04 20:00:00' and supply_id =1


alter table nadx_overall_dwr_v2 drop partition(b_time='2019-06-04 00:00:00');
alter table nadx_overall_dwr_v2 drop partition(b_time='2019-06-04 07:00:00');
alter table nadx_overall_dwr_v2 drop partition(b_time='2019-06-04 08:00:00');
alter table nadx_overall_dwr_v2 drop partition(b_time='2019-06-04 10:00:00');
alter table nadx_overall_dwr_v2 drop partition(b_time='2019-06-04 13:00:00');
alter table nadx_overall_dwr_v2 drop partition(b_time='2019-06-04 20:00:00');