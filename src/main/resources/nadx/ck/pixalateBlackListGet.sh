...
sleep 10s
    if [ -f $deviceIdFile ]; then
       ...
       ssh ck001 "clickhouse-client  -m  --password $CC_PASS --query='create table blacklist_device_id_raw_select_all as blacklist_device_id_raw ENGINE = Distributed(bip_ck_cluster, default, blacklist_device_id_raw, rand())'"
       ssh ck002 "clickhouse-client  -m  --password $CC_PASS --query='create table blacklist_device_id_raw_select_all as blacklist_device_id_raw ENGINE = Distributed(bip_ck_cluster, default, blacklist_device_id_raw, rand())'"
...
sleep 10s
  if [ -f $IPFile ]; then
      ...
      ssh ck001 "clickhouse-client  -m  --password $CC_PASS --query='create table blacklist_ip_raw_select_all as blacklist_ip_raw ENGINE = Distributed(bip_ck_cluster, default, blacklist_ip_raw, rand())'"
      ssh ck002 "clickhouse-client  -m  --password $CC_PASS --query='create table blacklist_ip_raw_select_all as blacklist_ip_raw ENGINE = Distributed(bip_ck_cluster, default, blacklist_ip_raw, rand())'"
