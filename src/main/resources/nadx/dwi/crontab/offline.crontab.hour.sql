...
drop table if exists nadx_performance_matched_dwi_tmp_for_hour;
create table nadx_performance_matched_dwi_tmp_for_hour as
select
  pDwi.repeats,
  pDwi.rowkey,
...
-- 去重
drop table if exists nadx_performance_matched_dwi_tmp_unrepeated_for_hour;
create table nadx_performance_matched_dwi_tmp_unrepeated_for_hour as
select
  repeats                          ,
  rowkey                            ,
...
from(
  ...
  from nadx_performance_matched_dwi_tmp_for_hour
)
where row_num = 1;
...
from (
  select * from nadx_performance_matched_dwi_tmp_unrepeated_for_hour where b_time >= ${start_b_time} and b_time <= ${end_b_time}
  UNION ALL
