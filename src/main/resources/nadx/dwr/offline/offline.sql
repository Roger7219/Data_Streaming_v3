...
left join (
  select
    ...
    b_time                              ,
    b_version
  from nadx_traffic_dwi_40000 where dataType = 4 AND b_time >= ${start_b_time} and b_time <= ${end_b_time}
...
left join (
  select
   -- 优化：只取需要的字段，降低性能损耗
    ...
    bidid    ,
    b_time
   from nadx_performance_dwi where repeated = 'N' AND b_time >= ${start_b_time} and b_time <= ${end_b_time} AND type = 'win'
